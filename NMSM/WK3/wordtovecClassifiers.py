...
svc_tfidf = Pipeline([("tfidf_vectorizer", TfidfVectorizer(analyzer=lambda x: x)), ("svc", SVC(kernel="linear",C=1000.0))])

# SVM + vec_from_dataset + MeanEmbedding
svc_w2v = Pipeline([("word2vec vectorizer", MeanEmbeddingVectorizer(w2v)), ("svc", SVC(kernel="linear",C=1000.0))])

# SVM + GLOVE vectors + MeanEmbedding
svc_glove_small = Pipeline([("glove vectorizer", MeanEmbeddingVectorizer(glove_small)), ("svc", SVC(kernel="linear",C=1000.0))])
